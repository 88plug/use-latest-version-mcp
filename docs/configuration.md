...
| `GITHUB_TOKEN` | _(unset)_ | Optional. Raises GitHub API rate limits for `github`, `ghcr`, and `swift`. Never required |
...
|---|---|---|
| `dry_run` | `true` | Preview only. Pass `false` to write manifests |
| `create_backup` | `true` | When writing, copy each file to `.dependency-backups/` first |

When it does write, each modified file is backed up first, and all edits for a
file roll back automatically if an error occurs. You opt in to writing
explicitly with `dry_run: false`.

See [Tool Reference → apply_upgrades](reference/tools.md#apply_upgrades) and
[Getting Started → Upgrade pipeline](getting-started.md#upgrade-pipeline-dry-run-by-default).
